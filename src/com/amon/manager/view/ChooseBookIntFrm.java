...
* 图书出借或者出售前，搜索图书
*/
public class ChooseBookIntFrm extends JInternalFrame {
	 ...
	 * Create the frame.
	 */
	public ChooseBookIntFrm(int userid) {
		this.userid=userid;
		setIconifiable(true);
		setMaximizable(true);
		setClosable(true);
		...
		JButton button = new JButton("租借出售");
	 ...
	 * @param e
	 */
	private void lengOrSealBookBtn(ActionEvent e) {
		int selectRow=table.getSelectedRow();//获取所选行
		if (selectRow==-1) {
			 JOptionPane.showMessageDialog(null, "请选择图书！");
			 return;
		}
		int bookid=(int) table.getValueAt(selectRow, 0);//获取所选行第1列（ID）的内容
		Book book=new Book(bookid);
		LendBookIntFrm lendBookIntFrm=new LendBookIntFrm(bookid,userid);
		lendBookIntFrm.setVisible(true);
		ManagerHomeView.table.add(lendBookIntFrm);
	}
