...
public class UpdateManagerInfo extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					UpdateManagerInfo frame = new UpdateManagerInfo(0);
	 ...
	 * Create the frame.
	 */
	public UpdateManagerInfo(int id) {
		this.userId=id;
		setResizable(false);
		setTitle("图书管理系统");
		...
		JLabel label_2 = new JLabel("\u4FEE\u6539\u7BA1\u7406\u5458\u4FE1\u606F");
	...
	Boolean getUserOldPWD(String newPWD){
		...
		try {
			...
			User currentUser=new User(userId,1);	//ID为1，用户类型为1（管理员）
